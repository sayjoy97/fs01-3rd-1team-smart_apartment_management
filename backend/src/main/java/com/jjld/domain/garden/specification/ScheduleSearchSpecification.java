    ...
    public static Specification<Schedule> withCondition(ScheduleSearchCondition cond) {
        return (root, query, cb) -> {
            ...
            if (cond.getName() != null && !cond.getName().isBlank()) {
                ...
                predicates.add(cb.like(gardenJoin.get("name"), "%" + cond.getName() + "%"));
            }

            if (cond.getAdminName() != null && !cond.getAdminName().isBlank()) {
                Join<Schedule, Admin> adminJoin = root.join("admin", JoinType.INNER);
                predicates.add(cb.like(adminJoin.get("adminName"), "%" + cond.getAdminName() + "%"));
            }

            if (cond.getWorkTitle() != null && !cond.getWorkTitle().isBlank()) {
                predicates.add(cb.like(root.<String>get("workTitle"), "%" + cond.getWorkTitle() + "%"));
            }

            if (cond.getWorkStartDate() != null && cond.getWorkEndDate() != null) {
                predicates.add(cb.lessThanOrEqualTo(root.get("workStartDate"), cond.getWorkStartDate()));

                predicates.add(cb.greaterThanOrEqualTo(root.get("workEndDate"), cond.getWorkEndDate()));
            }
            ...
            if (cond.getPriority() != null) {
                predicates.add(cb.equal(root.get("priority"), cond.getPriority()));
            }
